...
* 变形体的力学
  * 简单形状
    ...
    * 简单形状，复杂组合：结构力学，各种复杂桁架的受力分析（土木应该会经常用吧）
...
这是我踩的第一个坑，我想了10分钟才明白为什么是u的二阶导数。这里并不是位移-速度-加速度。 首先视频里说“长度上受到均布载荷P”，我理所应当的认为P是压强，应变也就是F/A，其实不是。
...
一般这种用一个（组）函数的线性和逼近另一个函数，一般考虑最小二乘法，这里把一个函数和另一个函数乘起来积分等于零（而且还不要求试函数组正交），用这种方法求系数的我还没见过。

经过了解，这个方法叫Galerkin（伽辽金）加权残值法（[Wiki 伽辽金法](https://zh.wikipedia.org/wiki/%E4%BC%BD%E8%BE%BD%E9%87%91%E6%B3%95)），简单地说，就是用试函数的基函数作为权重函数，就能取得系数。
