...
首先我们想办法把$$\sum_{u \in U}{\sum_{v \in V}{c_{uv}}}$$拆成正样本负样本两块，$$\sum_{u \in U}\sum_{v \in V^+}{c_{uv}}$$简写为$$\sum_{pos}$$， $$\sum_{u \in U}\sum_{v \in V^-}{c_{-}}$$简写为$$\sum_{neg}$$，其中，$$c_{-}$$是给所有负样本赋的统一的值。
...
最后，作者的模型里是有h（最后的一层线性层）的，但是从模型看这个h完全就是冗余的，实测效果是有没有h都没差。
